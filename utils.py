...
class Mapping:
    destination_type = {
        ...
        "SNOWFLAKE": "sink_consumers_snowflake_file_details",
...
class Utils:
    @staticmethod
    def get_dest_objects(int_id: int, dest_id: int) -> list:
        query = f"select dest_table_name from sink_consumers_table_mappings where integration_id={int_id} and " \
                f"destination_id={dest_id} "
        return Querier.fetch_service_data(query)

    @staticmethod
    def get_src_objects(int_id: int) -> list:
        query = f"select key_level0,key_level1 from integrations_source_objects where integration_id={int_id} and status='ACTIVE'"
        return Querier.fetch_service_data(query)

    @staticmethod
    def get_integration_id(pipeline_no: int, cluster_name: str, account_name: str) -> int:
        team_id = Utils.get_team_details(cluster_name, account_name)['id']
        query = f'select id from config_integrations where team_id={team_id} and seq_id={pipeline_no}'
    ...
    def get_team_details(cluster_name: str, account_name: str) -> dict:
        ...
        account_name = 'test'
        query = f"select id from security_teams where cluster_id='{cluster_name}' and name='{account_name}'"
        return Querier.fetch_groot_data(query)[0]

    @staticmethod
    def get_connector_task_details(int_id: int, task_name: list) -> list:
        columns = 'name,' \
                  'last_processed_ts,' \
                  'offset,' \
                  'meta,' \
        ...
        query = f"select {columns}, {case_list}  from connectors_tasks where integration_id={int_id} {condition} order by status"

        return Querier.fetch_service_data(query)

    @staticmethod
    def get_integration_details(int_id: int) -> dict:
        url = os.getenv('ENV_URL') + os.getenv('INTEGRATION_ENDPOINT').format(integration_id=int_id)
        headers = {
            'Authorization': 'Basic aW50ZWdyYXRpb25zOmludGVncmF0aW9uc0BoZXZv'
        }
    ...
    @staticmethod
    def get_handyman_tasks_details(int_id: int, task_name: list) -> dict:
        columns = 'id,' \
                  'unique_id,' \
                  'status,' \
                  'retry_count,' \
                  ...
                  'created_ts,' \
                  'processed_ts,' \
                  'timestampdiff(SECOND,created_ts,processed_ts) as time_diff_in_secs,' \
                  'rate_limiting_key'
        task_filter = ''
        if len(task_name) == 0:
        ...
        query = f"select {columns}  " \
                ...
                f"ht1.*," \
                f"(SELECT COUNT(*) " \
                f"FROM handyman_tasks ht2 WHERE " \
                f"ht2.unique_id = ht1.unique_id AND ht2.id >= ht1.id ) AS task_row_number " \
                f"from handyman_tasks ht1 " \
                f"where type ='connector_poll_task_job' and ({task_filter[:-2]}) " \
                f"or ht1.unique_id like '{int_id}-BinLog' " \
                f"or ht1.unique_id like '{int_id}-WAL' " \
                f"or ht1.unique_id like '{int_id}-Change Stream' " \
                f"or ht1.unique_id like '{int_id}-local.oplog.rs' ) as subquery WHERE task_row_number<4 " \
                f"order by id,unique_id desc"
        return Querier.fetch_service_data(query)

    @staticmethod
    def get_handyman_load_tasks_details(int_id: int, task_name: list) -> dict:
        columns = 'id,' \
                  'unique_id,' \
                  'status,' \
                  'retry_count,' \
        ...
        query = f"select {columns}  " \
                ...
                f" ) as subquery WHERE task_row_number<4 " \
                f"order by id,unique_id desc"
        return Querier.fetch_service_data(query)

    @staticmethod
    def get_sideline_details(int_id: int) -> list:
        columns = 'schema_name,' \
                  'stage,' \
                  'reason,' \
        ...
        case_list = case_list + 'ELSE Null END AS status_description'
    ...
    @staticmethod
    def get_sink_consumer_details(dest_id: int, dest_type: str, table_name: list) -> list:
    ...
    @staticmethod
    def get_destination_topic_info(topic_id: int) -> list:
        columns = 'partitions,' \
                  'name,' \
                  'consumer_group_type,' \
                  'exclusive_group,' \
                  'exclusive_group_id'
        query = f'select {columns} from config_independent_topic_info where id={topic_id}'
        return Querier.fetch_service_data(query)

    @staticmethod
    def get_destination_details(dest_id: int) -> list:
        columns = 'topic_id'
        query = f'select {columns} from config_destinations where id={dest_id}'
        return Querier.fetch_service_data(query)
...
class Analysis:
    @staticmethod
    def fetch_current_handyman_limit(rate_limiting_key: str) -> int:
        key = rate_limiting_key.split('|')
        query = f"select upper_limit from config_resource_utilization_overrides where constraining_key='{key[0]}' and " \
                f"`key`='{key[1]}' "
        result=Querier.fetch_service_data(query)
        if len(result)==0:
            return 14
    ...
    @staticmethod
    def fetch_stalled_task_count(team_id: int, rate_limiting_key)-> int:
        team_id=1
        query=f"select count(*) as cnt from handyman_tasks where team_id ={team_id}  and rate_limiting_key = '{rate_limiting_key}' and status = 'STALLED'"
        result=Querier.fetch_service_data(query)
        return result[0]['cnt']

    @staticmethod
    def fetch_all_task_count(team_id:int, type:str, rate_limiting_key)-> list:
        if type=='wal':
            query=f"select  count(DISTINCT entity_id) as total_task from handyman_tasks where team_id={team_id} and rate_limiting_key='{rate_limiting_key}' and type= 'connector_poll_task_job'"
        else:
            query=f"select count(DISTINCT unique_id) as total_task from handyman_tasks where team_id={team_id} and rate_limiting_key='{rate_limiting_key}' and type= 'connector_poll_task_job'"
